...
class Solution:
    def rotateGrid(self, grid: List[List[int]], k: int) -> List[List[int]]:
        ...
        while top < bottom and left < right:
            layer = []
            for j in range(left, right): layer.append(grid[top][j])     
            for i in range(top, bottom): layer.append(grid[i][right])   
            for j in range(right, left, -1): layer.append(grid[bottom][j]) 
            for i in range(bottom, top, -1): layer.append(grid[i][left])   
